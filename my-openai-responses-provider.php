	...
	if ( version_compare( phpversion(), MY_OPENAI_RESPONSES_PROVIDER_MIN_PHP_VERSION, '<' ) ) {
		add_action(
			...
			static function () {
				requirement_notice(
					sprintf(
						__( 'The Custom AI Provider plugin requires PHP version %1$s or higher. You are running PHP version %2$s.', 'my-openai-responses-provider' ),
	...
	if ( ! is_wp_version_compatible( MY_OPENAI_RESPONSES_PROVIDER_MIN_WP_VERSION ) ) {
		add_action(
			...
			static function () {
				...
				requirement_notice(
					sprintf(
						__( 'The Custom AI Provider plugin requires WordPress version %1$s or higher. You are running WordPress version %2$s.', 'my-openai-responses-provider' ),
...
function check_ai_client(): bool {
	if ( ! class_exists( \WordPress\AiClient\AiClient::class ) ) {
		add_action(
			...
			static function () {
				requirement_notice(
					__( 'The Custom AI Provider plugin requires the WordPress AI Client to be available in WordPress 7.0+.', 'my-openai-responses-provider' )
				);
			}
		);
